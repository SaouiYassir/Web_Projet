...
const product = new_products.find((p) => p.product_id == productId);

document.title = product.piece_Rechange + ` / ${product.carName} `;
document.querySelector(".banner h4").innerText = `Home > New Car Spare Parts > ${product.carName} > ${product.piece_Rechange}`
